...
The use of neurons in first hidden layer is to detect whether there's some kind of pattens in the input images. Since some patterns are smaller than the whole image, for example the beak of the bird, it doesn't have to scan the whole image to find out whether the pattern exists but just looking a small part of the image(red square).
...
The interpretation of the image will not be influenced if we shrink the size of the image, for example: remove the pixels of odd columns and even rows.
